...
The first three modules worked with individual files --- a CSV placed in a folder, a memo pasted into a conversation.  Most real business data does not live in spreadsheets.  It lives in databases: structured collections of related tables, hosted on servers, queried with SQL.

This module connects AI to those databases.  By the end, you will understand how your organization can create tools so that everyone can query a database in plain English --- no SQL required.  As throughout this course, the examples use Claude Code, but the same workflows apply to Gemini CLI and OpenAI Codex.

## From spreadsheets to databases

In Module 3 you analyzed a spreadsheet --- a single table of rows and columns living in a file on your machine.  Most real business data lives in databases: many related tables linked by keys, hosted on a server, queried with SQL.
...
The demo apps call Claude Sonnet through the Anthropic API.  If you have not already set up your API key, follow the instructions in Module 2 ("Setting up the Anthropic API").  The key must be available as an environment variable:
