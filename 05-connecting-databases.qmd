...
Chapter 1 described the vision: replacing dashboards with natural-language queries.  This chapter makes it real.  You will connect a coding agent to a database and build the tools that let anyone query it in plain English --- no SQL required.
...
In Chapter 3 you analyzed a spreadsheet --- a single table of rows and columns living in a file on your machine.  Most real business data lives in databases: many related tables linked by keys, hosted on a server, queried with SQL.
...
The exercises in this chapter use the Chinook sample database, which you can download here: [chinook.db](files/data/chinook.db).  It represents a music retailer's operations and contains 11 related tables.
...
In production, this app would be deployed on the corporate intranet, the database would be a PostgreSQL or Snowflake instance on the company's servers, and the LLM would be an enterprise-licensed model (more on licensing and compliance in Chapter 8).  For this demo, we run everything locally: a small Streamlit app on your laptop, the Chinook SQLite file in the same folder, and Claude Sonnet via the Anthropic API.

### Setting up the demo

The demo apps call Claude Sonnet through the Anthropic API.  If you have not already set up your API key, follow the instructions in Chapter 2 ("Setting up the Anthropic API").  The key must be available as an environment variable:
