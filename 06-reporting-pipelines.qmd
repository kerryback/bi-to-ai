...
Chapter 1 introduced the agent loop --- plan, execute, observe, iterate --- and the concept of agents chaining tools together.  This chapter puts that pattern to work.  You will build a reporting pipeline that queries a database, generates charts, writes narrative, and assembles a PowerPoint deck, all from a single instruction.

## From query to report

In Chapter 5, the Chinook query app answered questions one at a time.  You asked "What are the top ten countries by revenue?" and got a table.  Useful, but a stakeholder does not want a table --- they want a slide deck with a chart, key findings, and a recommendation.
...
**Step 1: Query the database.**  The agent runs a SQL query against the Chinook database --- the same kind of query the Chapter 5 app generates.  The result is a pandas DataFrame: rows of data ready for analysis.
...
**Step 5: Deliver.**  In a Streamlit app, the chart and summary appear on screen, and a download button lets the user grab the PowerPoint file.  The same pattern from Chapter 5 --- Streamlit as the web wrapper --- but the output is a document instead of a table.
...
Notice the last line of the prompt: logging.  Every LLM request and response is written to a JSONL file with timestamps.  This creates an audit trail --- you can see exactly what the LLM was asked and what it returned.  Chapter 8 will use these logs for verification and governance.
...
Open your coding agent in a folder containing the [Chinook database](files/data/chinook.db) and your SKILL.md from Chapter 5.  Give it a single compound instruction that requires at least three steps.
